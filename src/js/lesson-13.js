...
console.log("books_before:", bookShelf11.getBooks()); //! ['The Last Kingdom']

bookShelf11.addBook("The Mist");
bookShelf11.addBook("Dream Guardian");
console.log("books_add:", bookShelf11.getBooks()); //! ['The Last Kingdom', 'The Mist', 'Dream Guardian']

bookShelf11.removeBook("The Mist");
console.log("books_remove:", bookShelf11.getBooks()); //! ['The Last Kingdom', 'Dream Guardian']
console.log("---------------------------------------------------------------------");
...
const makingCoffeeNew = {
    ...
    //  coffee,
    makesGroundCoffee(W) {
        console.log("1️⃣ Мелена кава є?", Boolean(W));
        if (W) {
            // повертаємо рядок "W міліграм меленої кави"
            return  `${W} міліграм меленої кави`
        }
        // return W
    },

    makesHotWater(V) {
        console.log("2️⃣ Окріп (гаряча вода) є?", Boolean(V));
        if (V) {
            return `${V} мілілітрів гарячої води`
        }
        // return V
    },

    makingCoffee(Weight, volume) {
        // console.log("makesGroundCoffee(W):", this.makesGroundCoffee(Weight))
        // console.log("кількість зернової кави:", Weight, "міліграм")
        // console.log("кількість води:", volume, "мілімітрів")
        
        if (!(this.makesGroundCoffee(Weight))) {
            return "Потрібна мелена кава!";
        };

        if (!(this.makesHotWater(volume))) {
            return "Потрібна гаряча вода!";
        };
        const plus = " + "

        const coffee = this.makesGroundCoffee(Weight) + plus +  this.makesHotWater(volume)
        // console.log("coffee:", coffee)
        // coffee = "✅ Кава готова!"
        console.log("coffee:", coffee)
        // this.makesGroundCoffee(Weight) 
        // this.makesHotWater(volume)
        return coffee

    }
};

console.log("makingCoffeeNew-1:", makingCoffeeNew.makingCoffee(30, 250));
console.log("makingCoffeeNew-2:", makingCoffeeNew.makingCoffee(0, 0));
console.log("makingCoffeeNew-3:", makingCoffeeNew.makingCoffee());