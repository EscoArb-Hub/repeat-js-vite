...
const buttonBox = document.querySelector("#buttons-box2");
...
button1.addEventListener("click", (event) => {
    event.stopPropagation(); //! перешкоджає просуванню події далі
    console.log('Клік в "Button-1"', event.target);
    event.target.classList.toggle("active");
...
button2.addEventListener("click", (event) => {
    ...
    event.target.classList.toggle("active");
    console.log('Клік в "Button-2"', event.target);
});

button3.addEventListener("click", (event) => {
    // event.stopPropagation(); //! перешкоджає просуванню події далі
    console.log('Клік в "Button-3"', event.target);
    event.target.classList.toggle("active");
...
const buttonBox2 = document.querySelector("#buttons-box"); //! 🔸1️⃣

console.log("Buttons box:", buttonBox2);

buttonBox2.addEventListener("click", (event) => { //! 🔸2️⃣
...
//  при кліку на забраження картки вивести рік випуску в елементі тег <P> нижче інпуту
...
const findYear1 = document.querySelector(".find-year")
console.log("findYear1:", findYear1)

// const findImg = document.querySelector(".find-img")
// console.log("findImg:", findImg)

const findBriefName = (event) => {
    const resultInput = event.currentTarget.value;
    console.log("resultInput:", resultInput)

    // const arrBriefNames = aircrafts.map(elements => elements.name.brief)
    // console.log("arrBriefNames:", arrBriefNames)

    // const arrFilterNames = arrBriefNames.filter(element => element.toUpperCase().startsWith(resultInput.toUpperCase()))
    // console.log("arrFilterNames:", arrFilterNames)

    const arrResult = aircrafts.filter(element => element.name.brief.toLocaleUpperCase().startsWith(resultInput.toUpperCase()))
...
const findBriefName2 = (name) => {
    const resultInput = name
    console.log("resultInput:", resultInput)
...
function markup1(array) {
    console.log("array", array)
    const markup1 = array
        .map((item) =>
            `
            <li class="find-item" data-id="${item.id}">
                <h3 class="find-title">${item.name.brief}</h3>
                <img class="find-img" src="${item.url.main}" alt="${item.name.brief}">
                <p class="description">${item.info.description}</p>
            </li>
            `
        )
        .join("");
    findList.innerHTML = markup1
}

findName.addEventListener("input", findBriefName)
// findName.addEventListener("change", findBriefName)
markup1(aircrafts)


findList.addEventListener("click", findYear)
...
function findYear() {
    ...
    // 2 знаходимо батька img, це element (li)
    //  у якого є data-id="" в якому записаний id
    //  об'єкта який нам потрібен для пошуку року
    const card = image.parentNode
    console.log("card", card)
    // 3 знайти id
    const cardId = card.dataset.id
    console.log("cardId:", cardId)
    // 4 перебрати масив літаків та знайти об'єкт (літак) з таким id
    // const findAircraft = aircrafts.find(element => element.id === cardId) //! пошук по id 
    const findAircraft = aircrafts.find(element => element.url.main === link) //! пошук по link 
    console.log("findAircraft:", findAircraft)
    // 5 витягнути із знайденого об'єкта info.year
    const yearPlane = findAircraft.info.year
    console.log(":yearPlane", yearPlane)
    // 6 передати його в якості контенту в наш тег <p>
    findYear1.textContent = `Рік випуску: ${yearPlane}`
    //! тз 2 працюємо з інпутом

    const nameForInput = findAircraft.name.brief
    console.log("nameForInput:", nameForInput)

    // const findImg = document.querySelector(".find-img")
    // console.log("findImg:", findImg)

    // findName.focus()
    findName.value = nameForInput
    // // findName.blur()
    findBriefName2(nameForInput)

    // findImg.addEventListener("click", () => {
    //     findName.blur()
    // });
}
...
// План дій
// 1 клік в img
// 2 знаходимо батька img, це element (li) у якого є data-id="" в якому записаний id об'єкта який нам потрібен для пошуку року
// 3 знайти id
// 4 перебрати масив літаків та знайти об'єкт (літак) з таким id
// 5 витягнути із знайденого об'єкта info.year
// 6 передати його в якості контенту в наш тег <p>
